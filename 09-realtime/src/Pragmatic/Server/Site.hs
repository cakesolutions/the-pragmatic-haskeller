...
handleDataGet :: AppHandler ()
handleDataGet = do
    sample <- liftIO getSample
...
routes = basicRoutes ++ dslRoutes ++ puppyRoutes ++ fayRoutes


-------------------------------------------------------------------------------
fayRoutes :: [(ByteString, AppHandler ())]
fayRoutes = [ ("/hellofay", with fay $ render "hello_fay")
